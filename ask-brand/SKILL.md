...
- 先按目标路径调用 `setup-brand-ugc` 做只读预检；由 Agent 执行检查和无需凭证、无需付费的初始化，不要只把一组命令丢给用户。
- 缺少凭证时，先运行对应的凭证状态命令。若运行环境能展示可交互终端，由 Agent 启动安全输入命令，用户只在终端输入 Key；否则只提供一条可直接运行的安全输入命令。
- 用户输入完成后，由 Agent 自动复查凭证状态并恢复原下游任务；不要要求用户自行运行状态检查、只回复固定口令或重新描述原需求。
- 用户说“帮我配置”时，继续完成上述流程。只有密钥本身必须由用户在可信终端输入，其他可安全自动执行的步骤都由 Agent 完成。
- 如果真实 Key 曾被粘贴到聊天、日志或截图中，不使用也不保存；要求先撤销并生成新 Key，再进入安全终端输入流程。
