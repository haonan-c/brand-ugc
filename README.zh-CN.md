...
它还会在你同意后，把一段 brand-ugc 使用引导写进项目的 `AGENTS.md`（不存在就创建），
说明各个 Skill 的分工、品牌档案与凭证位置、产出目录约定，让之后的新会话不用重新解释
这套 Skills；同时创建或更新 `CLAUDE.md`，里面只放一行 `@AGENTS.md` 导入，Claude Code
读到的是同一份引导，不用维护两份副本。两处区块都由 `<!-- brand-ugc:start -->` 标记
包裹，重复运行只更新区块内部，不会动你自己写的章节。
...
`insights.json` 保存会持续演进的消费者洞察：人群痛点、使用场景、购买顾虑、真实
用户语言和内容支柱。它来自三个通道——运营访谈、本地素材和选题雷达回流——每个通道
都先产出补丁，确认后才合并。同一条洞察被多个通道重复观察到时，置信度自动上升。
存在洞察时，图文的封面钩子必须指向其中一条痛点或内容支柱。
